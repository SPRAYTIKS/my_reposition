        ...
        for anim in anim_type:
            ...
            for i in range(1, num_files + 1):
                ...
                img = pygame.transform.scale(img,
                                             (int(img.get_width() * sow), int(img.get_height() * sow)))
                temp.append(img)
...
def persona(n):
    ...
    if character1[n] in ['Fighter', 'Samurai', 'Shinobi']:
        plaer = Player(character1[n], 850, 300, True)
    elif character1[n] in ['Samurai_Archer']:
        plaer = Player(character1[n], 850, 300, True)
    elif character1[n] in ['Kunoichi']:
        plaer = Player(character1[n], 850, 270, True)
    elif character1[n] in ['Ninja_Monk']:
        plaer = Player(character1[n], 880, 320, True)
    else:
        plaer = Player(character1[n], 800, 270, True)
